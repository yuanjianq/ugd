...
function querygirdPartPage(currentPage){
	...
	//通过ajax查询后台数据
	RptManager.genReportDataToHtmlAndCount(rtpparams,'query_device_storage.rptdisger', function(content){
...
function initStorage(){
	...
	channel_id = selectValue;
		alert(channel_id);
	query();
}
function skipDetail(dataTime,storage_id,storageName){
	var loadURL = '/frame/business/device_detail.jsp?';
	var urlParam = "dataTime="+dataTime+"&dataType="+dateType+"&storageID="+storage_id+"&storageName="+storageName;
	jQuery.ajax({
		type : "POST",
