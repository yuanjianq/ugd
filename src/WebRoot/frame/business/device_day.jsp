...
<div class="portlet box blue">
	...
	<div class="portlet-body flip-scroll">
		...
		<select style="height:20px;width: auto;padding:0px;margin-bottom: 4px;"
			...
			查询日期：
			<input type="text" id="startTime1" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="tabledate Wdate" value="<%=DateUtil.getStringDate(-14)%>" class="Wdate" style="width:110px;height:18px;padding: 2px;margin-bottom:4px;line-height:normal;"/> 
			...
			<input type="text" id="endTime2" onfocus="WdatePicker({isShowWeek:true,disabledDays:[0,2,3,4,5,6]})" class="Wdate tabledate" value="<%=DateUtil.getStringWeek(0)%>" style="display: none;" class="Wdate"/> 
			<input type="text" id="endTime3" onfocus="WdatePicker({dateFmt:'yyyy-MM'})" class="Wdate tabledate" value="<%=DateUtil.getStringMonth(0)%>" style="display: none;" class="Wdate"/> 
...
function downExcelFile(){
	...
	RptManager.genReportFile(rtpparams,'query_device_day.rptdisger',{
...
function skipDetail(dataTime){
	var loadURL = '/frame/business/device_detail.jsp?';
	var urlParam = "dataTime="+dataTime+"&dataType="+dateType;
	jQuery.ajax({
		type : "POST",
...
function querygirdPartPage(currentPage){
	...
	//通过ajax查询后台数据
	RptManager.genReportDataToHtmlAndCount(rtpparams,'query_device_day.rptdisger', function(content){
