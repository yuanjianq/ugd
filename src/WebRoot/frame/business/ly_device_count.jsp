...
function downExcelFile(){
	...
	RptManager.genReportFile(rtpparams,'ly_query_detail_count.rptdisger',{
...
function querygirdPartPage(currentPage){
	...
	//通过ajax查询后台数据
	RptManager.genReportDataToHtmlAndCount(rtpparams,'ly_query_detail_count.rptdisger', function(content){
		parampage.count=parseInt(content[1]);
		//构造报表模板参数，并展示模板
		var param = new girdbirt01param(content[0],parampage);
		RptManager.genReportDataToHtml(rtpparams,'ly_query_detail_count_total.rptdisger', function(content){
			//构造报表模板参数，并展示模板
...
function initProducts(){
	var rtpparams = {};
	rtpparams.user_id =userID;
	RptManager.genReportDataToHtml(rtpparams,'select_product_ly.rptdisger', function(content){
