...
<div class="portlet box blue">
	<div class="portlet-title">
		<div class="caption" style="height: 20px;">
			<i class="icon-reorder"></i>天音手机型号实时数据
		</div>
		<div class="tools">
...
function downExcelFile(){
	...
	RptManager.genReportFile(rtpparams,'ty_query_model_rt.rptdisger',{
...
//跳转到渠道详细
function skipDetail(tagName,brand,model){
	var loadURL = '/frame/business/ty_rt_detail.jsp?';
	var urlParam = "boxId="+tagName+"&brand="+brand+"&model="+model;
	jQuery.ajax({
		type : "POST",
...
function querygirdPartPage(currentPage){
	...
	//通过ajax查询后台数据
	RptManager.genReportDataToHtmlAndCount(rtpparams,'ty_query_model_rt.rptdisger', function(content){
		parampage.count=parseInt(content[1]);
		//构造报表模板参数，并展示模板
		var param = new girdbirt01param(content[0],parampage);
		param.showLink={0:{name:'skipDetail',arg:[0,1,2]}};
